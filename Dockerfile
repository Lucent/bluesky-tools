...
# archive, assembles the static /site, and writes a brotli -q11 sibling (<file>.br) beside
# every text asset -- serve.py answers br-accepting clients from those, so compression
# happens once, at build; (3) the runtime image carries only /site + python.
...
RUN mkdir -p /site/flexforest && \
    ...
    find /site -type f \( -name '*.json' -o -name '*.js' -o -name '*.css' -o -name '*.html' \) -exec brotli -q 11 {} \;
...
USER nobody
EXPOSE 8000
# Static server with a content ETag + 304s and brotli negotiation over the build-time .br
# siblings (see serve.py) so a redeploy never serves a stale module from cache and no
# compression happens at runtime; Cloudflare is set to respect these origin headers.
CMD ["python", "/serve.py"]
